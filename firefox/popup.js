document.addEventListener('DOMContentLoaded', function () {
    ...
    browser.storage.sync.get('geminiApiKey', function (data) {
...
saveApiKeyButton.addEventListener('click', async function () {
    ...
    askButton.addEventListener('click', function () {
        ...
        // Get the current page's content by executing a content script
        browser.tabs.query({ active: true, currentWindow: true }, function (tabs) {
            const activeTab = tabs[0];
            browser.scripting.executeScript({
                target: { tabId: activeTab.id },
                files: ['get_content.js'] // <--- CHANGE IS HERE
            }, (results) => {
                        ...
                        browser.tabs.query({ active: true, currentWindow: true }, function (tabs) {
                            browser.tabs.sendMessage(tabs[0].id, {
                                ...
                                if (browser.runtime.lastError) {
                                    outputArea.textContent = `Error: ${browser.runtime.lastError.message}`;
                                } else if (response) {
